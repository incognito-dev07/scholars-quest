...
function initializeInfiniteModeQuestions() {
  ...
  const questionsToUse = shuffledQuestions.slice(0, Math.min(100, shuffledQuestions.length));
  
  questionsToUse.forEach((question, index) => {
    const infiniteQuestion = {...question};
    infiniteQuestion.id = 1000 + index;
    questions["Infinite Mode"].push(infiniteQuestion);
  });
  }
